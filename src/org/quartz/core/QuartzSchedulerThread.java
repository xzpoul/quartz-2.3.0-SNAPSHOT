...
public class QuartzSchedulerThread extends Thread {
                    ...
                    if (triggers != null && !triggers.isEmpty()) {
                        ...
                        while (timeUntilTrigger > 2) {
                            synchronized (sigLock) {
                                ...
                                //  只有在这样的情况下才值得
                                //  放弃触发器并获得一个新的触发器所需的时间成本
                                // 小于当前获取的触发器触发的时间，
                                //  否则，我们只是在“抖动”作业存储(例如数据库)。
                        ...
                        if (goAhead) {
                            try {
                                // 通知jobStore要执行Trigger,把jobDetails返回回来并组成一个TriggerFiredResultList
                        ...
                        // 循环执行trigger,创建JobRunShell(可执行的任务)
    ...
    private static long computeDelayForRepeatedErrors(JobStore jobStore, int acquiresFailed) {
        ...
        // 明智 sanity check per getAcquireRetryDelay specification
        if (delay < MIN_DELAY)
            delay = MIN_DELAY;
        if (delay > MAX_DELAY)
            delay = MAX_DELAY;
