...
public class SimpleThreadPool implements ThreadPool {
    ...
    public boolean runInThread(Runnable runnable) {
        ...
        /**
         * 关键字 synchronized 与 wait() 和 notify() / notifyAll() 方法相结合可以实现等待、通知模式，
         * 使用notify()/notifyAll()方法进行通知时，被通知的线程是由JVM随机选择的。
         * 而ReentrantLock结合Condition类可以实现“选择性通知”。
         */
        synchronized (nextRunnableLock) {
            ...
            while ((availWorkers.size() < 1) && !isShutdown) {
                try {
                    // xuzhou nextRunnableLock
                    // wait使线程停止运行
                    // 调用wait()之前,线程必须获得该对象的对象级别锁,即只能在同步方法或同步块中调用wait方法

                    // wait()方法可以使得调用该方法的线程 释放共享资源的锁,然后从运行状态退出,进入等待队列,直到被再次唤醒.
            ...
            // notify使停止的线程继续运行
            // notify使呈等待wait状态的线程退出wait状态,变成等待该对象上的锁(一旦该对象被解锁,他们就回去竞争)
            //  执行notifyAll()后,呈wait状态的线程并不能马上获取该对象锁,要等到执行notify()方法的线程将程序执行完,也就是退出
            // synchronized代码块后,当前线程才会释放锁,而呈wait状态所有的线程才可以获取该对象锁.

            // 如果没有使用notify语句,其他wait状态等待的线程由于没有得到该对象的通知,还需继续阻塞在wait状态,直到该对象发出notify()/notifyAll()的通知

            // 调用notifyAll()之前,线程必须获得该对象的对象级别锁,即只能在同步方法或同步块中调用notifyAll()方法

            // notify()随即唤醒等待队列中等待同一共享资源的"一个"线程,也仅通知"一个"线程.
            // notifyAll()唤醒等待队列中等待同一共享资源的"所有"线程,也仅通知"一个"线程.(优先级最高的那个线程最先执行,也可能随即执行,这取决于JVM虚拟机的实现)
            nextRunnableLock.notifyAll();
